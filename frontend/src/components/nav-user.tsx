...
export function NavUser({
  ...
  const handleDeleteAccount = async () => {
    if (window.confirm("Are you sure you want to delete your account? This action cannot be undone.")) {
      try {
        setLoading(true);
        // Replace with your API endpoint for deleting the account
        await axios.delete(`http://localhost:8000/api/v1/owner/deleteOwner/${user._id}`);
                ...
                filteredOwners.map((owner) => (
                  <div key={owner._id} className="border border-gray-200 rounded-lg shadow-lg p-6 bg-white hover:shadow-xl transition-shadow duration-300">
                    ...
                    <div className="flex items-center justify-center mb-6">
                      {owner.logo ? (
                        <img
                          src={`http://localhost:8000/uploads/${owner.logo}`} // Corrected logo URL
                          alt="Company Logo"
